...
export async function CmsStatCards() {
  ...
  const stats = [
    { label: "Total Articles", value: total, icon: Newspaper, hint: "All content items", color: "text-[var(--foreground)]", bg: "bg-[var(--surface)]" },
  ...
  return (
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4 mb-4">
      {stats.map(({ label, value, icon: Icon, hint, color, bg }) => (
        <div key={label} className="rounded-xl border border-[var(--line)] bg-[var(--paper)] p-5 shadow-sm transition hover:shadow-md">
          <div className="flex items-start justify-between">
            <div>
              <div className="text-[12px] font-semibold uppercase tracking-[0.1em] text-[var(--muted-soft)]">{label}</div>
              <div className="mt-2 text-3xl font-bold tracking-tight text-[var(--navy)]">{value}</div>
            </div>
            <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${bg} ${color}`}>
              <Icon size={20} />
            </div>
          </div>
          <div className="mt-4 border-t border-[var(--line-soft)] pt-3 text-[12px] text-[var(--muted)]">{hint}</div>
        </div>
      ))}
    </section>
  );
